...
export default function Landing({isAuthenticated, setIsAuthenticated}) {
  ...
  useEffect(() => {
    async function getAndSetNumberAllTodo() {
      try{
        const response = await axios.get('http://demcehnko-aleks-blue-green-lb-1120576598.us-east-1.elb.amazonaws.com/api/todo/count', {
          headers: {
    ...
    async function getAndSetNumberAllTodoNotCompleted() {
      try{
        const response = await axios.get('http://demcehnko-aleks-blue-green-lb-1120576598.us-east-1.elb.amazonaws.com/api/todo/count?isCompleted=false', {
          headers: {
            'Authorization': `Bearer ${sessionStorage.getItem('token')}`,
          }
        });
	...
	return (
		<div className="text-center">
			<h1>Todo List Application(demchenko oleksander)</h1>
      {showErrorMessage()}
			{message}
		</div>
	)
